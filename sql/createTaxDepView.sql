CREATE OR REPLACE VIEW v_tax_depreciation AS
WITH elect AS (
  SELECT
    a.id                                 AS asset_id,
...
macrs AS (
  SELECT
    ...
    r.year_in_service,
    r.rate,
    -- build one row per year in service
...
calc AS (
  SELECT
    ...
    GREATEST(
      ...
      - LEAST(
          m.bonus_amount,
          m.purchase_price - LEAST(m.sec179_amt, m.purchase_price)
        ),
      0
    ) * m.rate AS macrs_dep
  FROM macrs m
...
SELECT
  ...
  SUM(ROUND(sec179_dep + bonus_dep + macrs_dep, 2))
    OVER (PARTITION BY asset_id ORDER BY depreciation_date) AS accum_dep
FROM calc
